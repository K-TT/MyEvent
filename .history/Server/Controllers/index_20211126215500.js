...
module.exports.displayProfilePage = (req, res, next) => {
    res.render('index', { title: 'Profile', UserModel: User, page: 'profile', username: req.user ? req.user.username : '' });
...
module.exports.processProfilePage= (req, res, next) => {
   ...
   User.find(({ displayName: req.user.displayName } (err, UserModel) =>{
        if(err)
        {
            console.log(err);
            res.end(err);
